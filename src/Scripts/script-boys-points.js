...
fetch('/src/Data_Sheets/pointtable_boys.xlsx')
  .then(response => {
    if (!response.ok) {
  ...
  .then(data => {
    ...
    groupA.forEach(team => {
      ...
      [team.name, team.played, team.won, team.drawn, team.lost, team.points, team.goal_scored, team.goal_conceded, team.penalty_points].forEach(value => {
